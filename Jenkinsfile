pipeline {
    ...
    stages {
       stage('Checkout') {
                   steps {
                       // Checkout the current branch dynamically using GIT_BRANCH environment variable
                       script {
    ...
    post {
        ...
        success {
            // Send notification or perform action if the build is successful
            echo 'Build completed successfully!'
        }
        failure {
            // Send notification or perform action if the build fails
            echo 'Build failed.'
        }
    }
}